        ...
        if (user) {
            const fetchOrder = async () => {
                ...
                try {
                    ...
                    console.log(token);
                    console.log({checkout_session: session_id});
                    const order_res = await fetch(`${API_URL}/orders/confirm`, {
                        method: 'POST',
            ...
            fetchOrder().catch(err => console.log(err));
        }
    }, [user])
...
function Success(props) {
    ...
    return (
        <main className="main order border-top">
            <Helmet>
            ...
            <div className="page-content pt-7 pb-10 mb-10">
                ...
                <div className="container mt-8">
                    {loading && (<div className="icon-box-content text-left">
                        <h5 className="icon-box-title font-weight-bold lh-1 mb-1">Loading</h5>
                    </div>)}
                    {!loading && order && (
                        <>
                                ...
                                <div className="icon-box d-inline-flex align-items-center">
                                    <div className="icon-box-icon mb-0">
                                        ...
                                        c0-0.7,0-1.4-0.1-2.1"></path>
                                                <polyline fill="none" strokeWidth="4" strokeLinecap="round"
                                                          strokeLinejoin="bevel" strokeMiterlimit="10" points="
                            ...
                            <h2 className="title title-simple text-left pt-4 font-weight-bold text-uppercase">Order
                                Details</h2>
                                    ...
                                    <tbody>
                                    {
                                        cartList.map(item =>
                                            <tr key={'order-' + item.name}>
                                                <td className="product-name">{item.name} <span> <i
                                                    className="fas fa-times"></i> {item.qty}</span></td>
                            ...
                            <ALink href="/shop" className="btn btn-icon-left btn-dark btn-back btn-rounded btn-md mb-4"><i
                                className="d-icon-arrow-left"></i> Back to List</ALink>
                        </>
                    )}
                </div>
            </div>
